...
import (
	...
	"github.com/phanletrunghieu/example-go/endpoints"
	bookDecode "github.com/phanletrunghieu/example-go/http/decode/json/book"
	categoryDecode "github.com/phanletrunghieu/example-go/http/decode/json/category"
	lendBookDecode "github.com/phanletrunghieu/example-go/http/decode/json/lend_book"
	userDecode "github.com/phanletrunghieu/example-go/http/decode/json/user"
)
...
func NewHTTPHandler(endpoints endpoints.Endpoints,
	...
	r.Route("/lend_books", func(r chi.Router) {
		r.Get("/", httptransport.NewServer(
		...
		r.Get("/{lend_book_id}", httptransport.NewServer(
		...
		r.Put("/{lend_book_id}", httptransport.NewServer(
			endpoints.UpdateLendBook,
			lendBookDecode.UpdateRequest,
			encodeResponse,
			options...,
		).ServeHTTP)
		r.Delete("/{boolend_book_id}", httptransport.NewServer(
